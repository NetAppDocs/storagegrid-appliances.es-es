...
---
= Verifica el componente a sustituir en el SG120 o SG1200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Antes de empezar
* Tienes el número de serie del appliance donde se debe reemplazar el componente.
...
Usa este procedimiento para identificar el dispositivo con hardware defectuoso y cuál de los componentes de hardware reemplazables no está funcionando correctamente. Los componentes que podrían identificarse para reemplazo pueden incluir:
...
+
.. Desde el Grid Manager, selecciona *Nodes*.
.. En la tabla de la página Nodos, selecciona el nombre del dispositivo StorageGRID con el componente que ha fallado.
...
+
*** Si no aparece la sección StorageGRID Appliance en Grid Manager, el nodo seleccionado no es un dispositivo StorageGRID. Selecciona un nodo diferente en la vista de árbol.
...
link:turning-sg120-and-sg1200-identify-led-on-and-off.html["Encienda el LED de identificación del aparato"]
