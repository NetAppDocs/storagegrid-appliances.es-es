...
---
= Cablea un dispositivo o controlador StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** De uno a cuatro cables Twinax para conectar los cuatro puertos de red.
** Para el SG120, de uno a cuatro transceptores SFP+ o SFP28 si planeas usar cables ópticos para los puertos.
** Para el SG1200, de uno a cuatro transceptores QSFP+, QSFP28 o QSFP56 (solo NIC 200GbE) si planeas usar cables ópticos para los puertos.
...
Conexiones de puerto SG120:

image::../media/sg120_connections.png[Conectores traseros SG120]

Conexiones de puerto SG1200:

image::../media/sg1200_connections.png[Conexiones de puerto SG1200]
...
* 10/25/40/100-GbE con NIC de 100G
* 10/25/50/100/200-GbE (solo 200GbE NIC)

 a| 
Conéctate a la red Grid y a la red cliente para StorageGRID. Consulta link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes["Modos de enlace de puertos (SG120 y SG1200)"]
...
[cols="2a,2a"]
|===
| Velocidad de enlace SG120 (GbE) | Equipo necesario 
...
[cols="2a,2a"]
|===
| Velocidad de enlace SG1200 (GbE) | Equipo necesario 
...
.Antes de empezar
* Sólo SG6200: tienes el cable multiconector de 100GbE a 4x25GbE suministrado con el dispositivo para conectar los dos controladores de almacenamiento al controlador SG6200-CN.
* Tiene cables Ethernet RJ-45:
+
** Un cable RJ-45 para conectar el puerto de administración.
** Sólo SG6200: Hasta cuatro cables Ethernet RJ-45 adicionales para los puertos opcionales que planees usar, incluidos el segundo puerto Admin y el puerto BMC en el SG6200-CN, y los puertos de mantenimiento en cada uno de los dos controladores E4000.


* Tiene una de las siguientes opciones para los puertos de red. Estos elementos no se suministran con el aparato.
+
** De uno a cuatro cables Twinax para conectar los cuatro puertos de red.
** De uno a ocho transceptores SFP+, SFP28, QSFP+, QSFP28 o QSFP56 si planeas usar cables ópticos para los puertos.
...
.Acerca de esta tarea
Las siguientes figuras muestran los puertos en la parte posterior del SGF6212 y los tres controladores en el appliance SG6200. El appliance SG6200 incluye un controlador de cómputo SG6200-CN en la parte superior y dos controladores de almacenamiento E4000 en la parte inferior.

Conexiones SGF6212:

image::../media/sgf6212_connections.png[Conectores traseros SGF6212]

Conexiones SG6260:

image::../media/sg6200_e4000_connections.png[Conexiones de SG6200-CN a E4000]
...
2
 a| 
Cuatro puertos de red en el controlador SG6200-CN
 a| 
* Cuatro puertos de red 10/25/40/100-GbE o 200-GbE con SKU NIC 200G opcional

 a| 
Conéctate a la red Grid y a la red cliente para StorageGRID. Ver link:gathering-installation-information-sg6200.html#port-bond-modes["Modos de enlace de puertos (SG6200)"]
...
Cinco puertos de conexión en total
 a| 
* Un puerto 100GbE en el SG6200-CN
* Dos puertos 10/25GbE en cada controladora de almacenamiento

 a| 
Conecta cada controlador de almacenamiento al controlador SG6200-CN.
...
. Conecta los dos puertos de interconexión de cada controlador de almacenamiento al puerto de 100GbE del controlador SG6200-CN, usando un cable QSFP28 de 100GbE a 4x25GbE SFP28.
. Conecte los puertos de red del dispositivo a los switches de red adecuados utilizando cables Twinax o cables ópticos y transceptores.
+
[cols="2a,2a"]
|===
...
. Si tiene previsto utilizar la Red de administración para StorageGRID, conecte el puerto Red de administración del dispositivo a la Red de administración mediante un cable Ethernet.
. Si tu aparato incluye una bandeja de expansión, consulta la link:../sg6200/adding-expansion-shelf-to-deployed-sg6260.html["instrucciones para añadir una bandeja de expansión a un SG6200 desplegado"] para obtener información sobre el cableado.
