...
summary: Los dispositivos de la serie StorageGRID SG6200 funcionan como nodos de almacenamiento en un sistema StorageGRID. Como todos los dispositivos StorageGRID, pueden mezclarse libremente con otros modelos de dispositivos y nodos exclusivamente de software en una única implementación. 
---
= Dispositivos StorageGRID SG6200
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Los dispositivos de la serie StorageGRID SG6200 funcionan como nodos de almacenamiento en un sistema StorageGRID. Como todos los dispositivos StorageGRID, pueden mezclarse libremente con otros modelos de dispositivos y nodos exclusivamente de software en una única implementación.

El dispositivo StorageGRID SG6260 incluye un controlador informático con un par de unidades SSD NVMe que funcionan como caché de lectura y un estante de controlador de almacenamiento que contiene dos controladores de almacenamiento y 60 unidades de disco duro NL-SAS. Se puede ampliar hasta 180 unidades de disco duro NL-SAS mediante la adición de hasta dos estantes de expansión opcionales. El dispositivo StorageGRID SGF6212 es un dispositivo all-flash con un factor de forma 1U compacto poblado con 12 unidades SSD NVMe.
...
Los dispositivos SGF6212 y SG6260 ofrecen las siguientes funciones:
...
El SGF6212 proporciona las siguientes funciones de protección de datos:
...
El SG6260 proporciona las siguientes funciones de protección de datos:
...
== Componentes de hardware del SG6200



=== Dispositivo SGF6212

El dispositivo SGF6212 incluye los siguientes componentes:

Plataforma de almacenamiento y computación:: Un servidor de una unidad de rack (1U) que incluye:
+
--
* 256 GB DE MEMORIA RAM
* unidad de arranque interna de 240 GB (incluye software StorageGRID)
* 2 x 1/10 puertos GBase-T.
* 4 puertos Ethernet 10/25/40/100GbE para tráfico de red Grid/Client (o 4 x 200GbE con NIC de 200GbE opcional)
* 12 unidades SSD NVMe para almacenamiento de datos
...
El dispositivo SG6260 incluye los siguientes componentes:

Controladora de computación:: El controlador SG6200-CN es un servidor de una unidad rack (1U) que incluye:
+
--
* 256 GB DE MEMORIA RAM
* unidad de arranque interna de 240 GB (incluye software StorageGRID)
* 2 x 1/10 puertos GBase-T.
* 4 puertos Ethernet 10/25/40/100GbE para tráfico de red Grid/Client (o 4 x 200GbE con NIC de 200GbE opcional)
* 1 puerto de interconexión de almacenamiento de 100 GbE
...
--
Opcional: Bandejas de ampliación del almacenamiento:: Cada dispositivo SG6260 puede tener una o dos bandejas de expansión para un total de 180 unidades. Las bandejas de expansión pueden instalarse durante el despliegue inicial o añadirse posteriormente.
+
--
...
== Diagramas SGF6212 y SG6260



=== SGF6212 vista frontal

Esta figura muestra el frontal del SGF6212 sin el frontal. El dispositivo incluye una plataforma informática y de almacenamiento 1U que contiene 12 unidades SSD.

image::../media/s25_front_with_ssds.png[SGF6212 Vista frontal]



=== SGF6212 vista trasera

Esta figura muestra la parte posterior del SGF6212, incluidos los puertos, ventiladores y fuentes de alimentación.

image::../media/sgf6212_rear_connectors.png[SGF6212 Vista trasera]
...
|===
Esta figura muestra la ubicación de la fuente de alimentación y los LED de identificación en la parte posterior del SGF6212. En los puertos del appliance hay LED de estado y actividad adicionales. Estos LED pueden variar según el modelo de appliance.

image::../media/s25_rear_leds.png[LEDs traseros SGF6212]
...
Esta figura muestra la parte frontal del SG6260, que incluye un controlador informático de 1U y un estante de 4U que contiene dos controladores de almacenamiento y 60 unidades en cinco cajones de unidades.
...
1
 a| 
Controlador informático SG6200-CN con el frontal desmontado
...
=== SG6260 vista trasera

Esta figura muestra la parte posterior del SG6260, incluidos los controladores de computación y almacenamiento, los ventiladores y las fuentes de alimentación.

image::../media/sg6260_rear_view.png[SG6260 Vista trasera]
...
1
 a| 
Fuente de alimentación (1 de 2) para controlador informático SG6200-CN
...
2
 a| 
Conectores para el controlador de cómputo SG6200-CN
...
4
 a| 
Controlador de almacenamiento E-Series E400 (1 de 2) y conectores
...
Esta figura muestra los puertos en la parte posterior del controlador informático SG6200-CN.

image::../media/sg6200_cn_rear_connectors.png[Conectores traseros SG6200-CN]
...
Puertos de red 1-4
 a| 
10/25/40/100/200-GbE, en función del tipo de cable o transceptor, la velocidad del conmutador y la velocidad de enlace configurada. Se admiten de forma nativa QSFP56 (máx. 200GbE/puerto), QSFP28 (máx. 100GbE/puerto) y QSFP+ (40GbE) (las velocidades de 200GbE requieren la opción de NIC de 200GbE). Se pueden utilizar transceptores SFP+ (10GbE) o SFP28 (25GbE) opcionales con un QSA (se venden por separado).
 a| 
Conéctese a la red de red y a la red de cliente para StorageGRID.
...
1 GbE (RJ-45).
 a| 
Conéctate al controlador de gestión de la placa base SG6200-CN.
...
1 U/10 GbE (RJ-45)
 a| 
Conecta el SG6200-CN a la red de administración de StorageGRID.
...
100 GbE
 a| 
Conecta el controlador SG6200-CN a los controladores E4000.

|===
Esta figura muestra la ubicación de la fuente de alimentación y los LED de identificación en la parte posterior del controlador de cómputo SG6200-CN. LED de estado y actividad adicionales están en los puertos del dispositivo. Estos LED pueden variar según el modelo de dispositivo.

image::../media/s25_rear_leds.png[LEDs traseros SG6200-CN]
...
=== SG6260: Controlador de almacenamiento E4000
...
* Conéctate al controlador SG6200-CN y proporciona acceso al almacenamiento.
...
25GbE iSCSI
 a| 
Conecta cada uno de los controladores E4000 al controlador SG6200-CN.

Hay cuatro conexiones al controlador SG6200-CN (dos desde cada E4000).

|===


=== SG6260: IOMs para bandejas de expansión opcionales
