...
---
= Soluciona problemas de instalación de hardware para un SG5800 o SG6100
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== [[view-boot-codes]]Ver códigos de arranque (solo SGF6112 y SG6160)
...
*Nota*: Si el procedimiento de arranque no avanza más allá de esta etapa, realice estos pasos:

.. Confirmar que los cuatro cables de interconexión entre la controladora SG6100-CN y las dos controladoras de almacenamiento están conectados de forma segura.
...
== [[view-error-codes]]Ver códigos de error (solo SGF6112 y SG6160)
...
.Pasos
. Revise los LED del aparato y/o los códigos de error y arranque mostrados en BMC (si está equipado).
...
=== Las bandejas de expansión no aparecen en el instalador de dispositivos (SG6160)

Si ha instalado bandejas de ampliación y no aparecen en el instalador de dispositivos de StorageGRID, debe comprobar que las bandejas se hayan instalado completamente y encendido.
...
* La página *Avanzado* > *Modo RAID* indica por número de unidades si el dispositivo incluye o no estantes de expansión.
...
. Verifique que se hayan encendido las bandejas de expansión. Consulte link:connecting-power-cords-and-applying-power.html["Conectar los cables de alimentación y aplicar alimentación (SG6100)"].
. Si necesita ayuda para resolver un problema, póngase en contacto con el soporte técnico.
...
Si no puede conectarse al dispositivo de almacenamiento, es posible que haya un problema de red o que la instalación de hardware no se haya completado correctamente.
...
. Introduzca la dirección URL del instalador de dispositivos de StorageGRID o del Administrador del sistema de SANtricity: +
