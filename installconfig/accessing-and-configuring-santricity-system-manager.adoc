...
---
= Configura SANtricity para los dispositivos StorageGRID SG6260, SG6160, SG6000, SG5700 y SG5800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Los siguientes dispositivos StorageGRID pueden acceder a SANtricity System Manager:
...
* Claves de seguridad de la unidad, necesarias para desbloquear las unidades protegidas
+

NOTE: Debes configurar y tener acceso a SANtricity System Manager si la función Drive Security está activada para tu dispositivo.
...
** Usa Grid Manager y selecciona *Nodes* > `*_appliance Storage Node_*` > *SANtricity System Manager*
...
.SG6260
--
El dispositivo de almacenamiento SG6260 puede equiparse con unidades compatibles con FIPS tanto en el controlador informático SG6200-CN como en la bandeja del controlador E4000. El cifrado de unidades se configura por separado para las unidades SG6200-CN y las unidades E4000.

.. link:optional-enabling-node-encryption.html#drive-encryption["Habilite el cifrado de unidades"] para unidades SSD SED instaladas en el nodo de computación SG6200-CN.
.. Crear una clave de seguridad (una cadena de caracteres compartida por la controladora y las unidades para acceso de lectura/escritura).
+
*** Es https://docs.netapp.com/us-en/e-series-santricity/sm-settings/create-internal-security-key.html["cree una clave interna"^] posible desde la memoria persistente de la controladora o usar una clave externa proporcionada por un servidor de gestión de claves.
*** Para usar una clave externa proporcionada por un servidor de gestión de claves, primero se debe https://docs.netapp.com/us-en/e-series-santricity/sm-settings/overview-drive-security.html["establezca la autenticación con un servidor de gestión de claves"^] en SANtricity System Manager.
...
NOTE: Sigue este procedimiento solo si no tienes acceso a SANtricity System Manager desde StorageGRID Appliance Installer (*Advanced* > *SANtricity System Manager*) o Grid Manager (*Nodes* > *SANtricity System Manager*).
