...
---
= Reemplaza una unidad en un SG100 o SG1000
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
También puedes usar el Grid Manager para supervisar el estado de las SSD. Selecciona *Nodes*. Luego selecciona `*Appliance Node*` > *Hardware*. Si una unidad ha fallado, el campo Storage RAID Mode contiene un mensaje sobre cuál unidad ha fallado.
...
+
La unidad se reconstruye automáticamente con datos reflejados de la unidad de trabajo. Puedes comprobar el estado de la reconstrucción usando el Grid Manager. Selecciona *Nodos*. Luego selecciona `*Appliance Node*` > *Hardware*. El campo Storage RAID Mode contiene un mensaje "`rebuilding`" hasta que la unidad se reconstruye por completo.
